...
public class MiniStatementServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Transaction> miniList = new ArrayList<>();
        ...
        try (Connection con = DBConnection.getConnection()) {
            ...
            String sql = "SELECT * FROM transactions WHERE user_id=? ORDER BY transaction_date DESC LIMIT 5";
            ...
            request.setAttribute("successMessage", "Mini statement loaded successfully.");
            request.getRequestDispatcher("miniStatement.jsp").forward(request, response);

        } catch (Exception e) {
            e.printStackTrace();
            request.setAttribute("errorMessage", "Mini statement error: " + e.getMessage());
            request.getRequestDispatcher("miniStatement.jsp").forward(request, response);
        }
    }
}